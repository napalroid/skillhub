...
<head>
    ...
    <title><?php echo e($service->title); ?> — SkillHub</title>
...
<body class="bg-[#f2f2f1] text-[#171717] antialiased pt-16">
    <div id="skillhub-staggered-menu"
         ...
         data-avatar-url="<?php echo e(auth()->user()?->avatar_url ?? ''); ?>"
          ...
          data-notifications-read-all-url="<?php echo e(auth()->check() ? route('notifications.read-all') : ''); ?>"
           ...
           data-csrf-token="<?php echo e(csrf_token()); ?>"></div>
    ...
    <div x-data="productPage(<?php echo \Illuminate\Support\Js::from($photos->values())->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($reviewData)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from(session('success') ?? session('error'))->toHtml() ?>)" @keydown.escape.window="closeAll()" class="min-h-screen">
        <header class="border-b border-black/10 bg-[#f2f2f1]">
            <div class="mx-auto flex h-14 max-w-[1440px] items-center justify-between px-5 sm:px-8">
                <a href="<?php echo e(route('services.index')); ?>" class="inline-flex items-center gap-1.5 text-[11px] font-semibold tracking-tight text-black/75 transition hover:text-black">
        ...
        <main class="mx-auto max-w-[1440px] px-5 py-9 sm:px-8 lg:py-12">
            <div class="grid gap-8 lg:grid-cols-[1.45fr_.85fr] lg:gap-12">
                ...
                <section class="pt-1">
                    <p class="text-xs font-semibold uppercase tracking-[.16em] text-black/45"><?php echo e($service->subcategory?->name ?? 'Jasa'); ?></p>
                    <h1 class="mt-2 text-3xl font-semibold tracking-[-.06em] sm:text-[34px]"><?php echo e($service->title); ?></h1>
                    <p class="mt-1.5 text-sm font-semibold">Rp<?php echo e(number_format($service->price, 0, ',', '.')); ?></p>
                    ...
                    <div class="mt-5 overflow-hidden rounded-[15px] border border-black/15 bg-[#f7f7f6]">
                        ...
                        <div x-show="descOpen" x-transition class="px-4 pb-4 text-xs leading-[1.55] text-black/60"><?php echo e($service->description); ?></div>
                    </div>

                    <div class="mt-5">
                        <p class="text-[11px] font-bold uppercase tracking-[.12em] text-black/55">Tentang jasa</p>
                        <div class="mt-2 flex flex-wrap gap-1.5">
                            <span class="rounded-full border border-black/15 px-3 py-1 text-[10px] font-medium"><?php echo e($service->subcategory?->category?->name ?? 'SkillHub'); ?></span>
                            <span class="rounded-full border border-black/15 px-3 py-1 text-[10px] font-medium"><?php echo e($service->subcategory?->name ?? 'Jasa siswa'); ?></span>
                            <span class="rounded-full border border-black/15 px-3 py-1 text-[10px] font-medium"><?php echo e($service->orders_count); ?> kali terjual</span>
                            <span class="rounded-full border border-black/15 px-3 py-1 text-[10px] font-medium"><?php echo e($reviewCount); ?> review</span>
                        </div>
                    </div>

                    <div class="mt-5 flex items-center gap-2">
                        <span class="text-sm font-bold"><?php echo e($service->seller?->name ?? 'Siswa SkillHub'); ?></span>
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->guard()->check()): ?>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->id() !== $service->user_id): ?>
                            <div class="mt-6 grid grid-cols-[1fr_1fr] gap-3">
                                <form method="POST" action="<?php echo e(route('conversations.start', $service)); ?>"><?php echo csrf_field(); ?><button type="submit" class="w-full rounded-full border border-black bg-transparent px-4 py-3 text-xs font-bold transition hover:bg-white" title="Buka ruang chat untuk mendiskusikan harga">Diskusikan harga?</button></form>
                                <a href="<?php echo e(route('orders.create', $service)); ?>" class="flex w-full items-center justify-center gap-2 rounded-full bg-[#171717] px-4 py-3 text-xs font-bold text-white transition hover:bg-black">Pesan sekarang</a>
                            </div>
                        <?php else: ?>
                            <div class="mt-6 grid grid-cols-[1fr_1fr] gap-3">
                                <a href="<?php echo e(route('conversations.seller-index')); ?>" class="flex w-full items-center justify-center gap-2 rounded-full border border-black bg-transparent px-4 py-3 text-xs font-bold transition hover:bg-white" title="Buka percakapan dengan pembeli">
                    ...
                    <?php else: ?>
                        <a href="<?php echo e(route('login')); ?>" class="mt-6 block rounded-full bg-[#171717] px-4 py-3 text-center text-xs font-bold text-white">Masuk untuk memesan</a>
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
            ...
            <section class="mt-12 grid gap-8 border-t border-black/15 pt-9 lg:grid-cols-[1.45fr_.85fr]">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->guard()->check()): ?>
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($userReview): ?>
                        <div class="col-span-full">
                            <div class="rounded-xl bg-green-50 border border-green-200 p-4">
                                ...
                                <div class="mt-2 flex items-center gap-2">
                                    <span class="font-bold"><?php echo e($userReview->rating); ?>/5</span>
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($userReview->is_verified_buyer): ?>
                                        <span class="text-xs bg-blue-100 text-blue-800 px-2 py-0.5 rounded">Verified Buyer</span>
                                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                </div>
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($userReview->comment): ?>
                                    <p class="mt-2 text-sm text-gray-700"><?php echo e($userReview->comment); ?></p>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </div>
                        </div>
                    <?php else: ?>
                        <div class="col-span-full">
                            <form method="POST" action="<?php echo e(route('reviews.store', $service)); ?>" class="rounded-xl bg-gray-50 border border-gray-200 p-4" x-data="{ rating: 0, hoverRating: 0 }">
                                <?php echo csrf_field(); ?>
                    ...
                    <div class="col-span-full">
                        <div class="rounded-xl bg-blue-50 border border-blue-200 p-4 text-center">
                            <p class="text-sm text-blue-800">
                                <a href="<?php echo e(route('login')); ?>" class="font-semibold underline">Login</a> untuk memberi review
                            </p>
                        </div>
                    </div>
                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                ...
                <div class="lg:col-span-1">
                    ...
                    <div class="mt-4 grid grid-cols-[auto_1fr] items-center gap-x-7">
                        <div><p class="text-6xl font-semibold tracking-[-.1em]"><?php echo e($averageRating); ?></p><p class="mt-1 text-[10px] text-black/45">(<?php echo e($reviewCount); ?> review)</p></div>
                ...
                <div x-data="reviewPanel(<?php echo \Illuminate\Support\Js::from($reviewData)->toHtml() ?>)" class="lg:col-span-1 border-t border-black/15 pt-6 lg:border-l lg:border-t-0 lg:pl-8 lg:pt-0">
        ...
        <div x-show="modalOpen" x-cloak x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center p-5" role="dialog" aria-modal="true">
            ...
            <div x-show="modalOpen" x-transition:enter="transition ease-out duration-250" x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100" class="relative w-full max-w-md rounded-[20px] bg-white p-6 shadow-2xl">
                ...
                <p class="mt-2 text-xs leading-5 text-black/55">Pesanmu akan diteruskan ke <?php echo e($service->seller?->name ?? 'penyedia jasa'); ?> dalam ruang pesanan.</p>
                <form method="POST" action="<?php echo e(route('orders.store')); ?>" class="mt-5" x-data="{ sending: false }" @submit="sending = true"><?php echo csrf_field(); ?><input type="hidden" name="service_id" value="<?php echo e($service->id); ?>"><textarea name="message" required maxlength="1000" placeholder="Halo, saya ingin mendiskusikan harga jasa ini..." class="h-28 w-full resize-none rounded-xl border border-black/15 bg-[#f6f6f5] p-3 text-xs outline-none focus:border-black"></textarea><button :disabled="sending" class="mt-3 flex w-full items-center justify-center gap-2 rounded-full bg-black px-4 py-3 text-xs font-bold text-white disabled:opacity-70"><i x-show="sending" x-cloak data-lucide="loader-circle" class="h-3.5 w-3.5 animate-spin"></i><span x-text="sending ? 'Mengirim...' : 'Kirim pesan'"></span></button></form>
            </div>
        </div>
        ...
        <div x-show="sidebarOpen" x-cloak class="fixed inset-0 z-50 lg:hidden">
            ...
            <aside x-show="sidebarOpen" x-transition:enter="transition ease-out duration-250" x-transition:enter-start="-translate-x-full" x-transition:enter-end="translate-x-0" class="relative h-full w-72 bg-white p-5">
                ...
                <nav class="mt-7 space-y-1 text-sm font-semibold">
                    <a href="<?php echo e(route('home')); ?>" class="block rounded-xl px-3 py-3 hover:bg-black/5">Beranda</a>
                    <a href="<?php echo e(route('services.index')); ?>" class="block rounded-xl px-3 py-3 hover:bg-black/5">Marketplace</a>
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->guard()->check()): ?>
                        <a href="<?php echo e(route('orders.index')); ?>" class="block rounded-xl px-3 py-3 hover:bg-black/5">Pesanan</a>
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                </nav>
            </aside>
        </div>
...
</html><?php /**PATH D:\xampp\htdocs\UKK MODE SERIUS\skillhub\resources\views/marketplace/show.blade.php ENDPATH**/ ?>