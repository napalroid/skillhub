...
<head>
    ...
    <title><?php echo e($title ?? 'SkillHub Admin'); ?></title>
    <?php echo app('Illuminate\Foundation\Vite')(['resources/css/app.css', 'resources/js/app.js', 'resources/js/routes/admin.js']); ?>
    <script>
        window.route = "<?php echo e(route('admin.dashboard')); ?>".replace('dashboard', '');
...
<?php endif; ?>
    ...
    <aside 
        ...
        <div class="flex items-center justify-between h-16 px-6 border-b border-white/10">
            <a href="<?php echo e(route('admin.dashboard')); ?>" class="flex items-center gap-2" aria-label="SkillHub Admin Home">
        ...
        <div class="p-4 border-t border-white/10">
            ...
            <a href="<?php echo e(route('profile.edit')); ?>" class="flex items-center gap-3 px-4 py-3 rounded-sm text-sm font-medium text-white/70 hover:text-white hover:bg-white/10 transition-colors">
    ...
    <div class="fixed top-0 right-0 z-30 h-16 bg-white border-b border-[#DDDDDD] flex items-center justify-between px-6 transition-all duration-300"
        ...
        <a href="<?php echo e(route('dashboard')); ?>" class="px-3 py-2 text-xs font-bold uppercase tracking-wide text-[#555555] hover:text-black transition-colors">Dashboard User</a>
    </div>
    ...
    <script>
        function adminLayout() {
            return {
                ...
                pageTitle: 'Dashboard',
                authUser: <?php echo json_encode(auth()->user() ?? [], JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT, 512) ?>,
                navItems: [
                    { route: '<?php echo e(route('admin.dashboard')); ?>', label: 'Dashboard', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 7v10a2 2 0 0 0 2 2h10l8-8V7a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2Z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M10 11l3-3 6 6"/></svg>' },
                    { route: '<?php echo e(route('admin.services.index')); ?>', label: 'Semua Jasa', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20.25 14.15c.864.864 1.545 2.05.864 3.297a4.643 4.643 0 01-1.323 2.426M15 18c0 2.485-2.015 4.5-4.5 4.5s-4.5-2.015-4.5-4.5M15 10.5A4.5 4.5 0 007.5 15M15 10.5A4.5 4.5 0 0122.5 15M15 10.5a4.5 4.5 0 11-9 0 4.5 4.5 0 019 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M2.25 10.5h1.5M2.25 15h1.5"/></svg>' },
                    { route: '<?php echo e(route('admin.services.pending')); ?>', label: 'Approval', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z"/></svg>' },
                     { route: '<?php echo e(route('admin.categories.index')); ?>', label: 'Kategori', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3.75 12h16.5M6 6h12v12.75a2.25 2.25 0 0 0 2.25 2.25H6A2.25 2.25 0 0 1 3.75 12z"/></svg>' },
                     { route: '<?php echo e(route('admin.escrow.index')); ?>', label: 'Escrow', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z"/></svg>' },
                    { route: '<?php echo e(route('admin.payouts.index')); ?>', label: 'Pencairan', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 6.75A2.25 2.25 0 0 1 5.25 4.5h13.5A2.25 2.25 0 0 1 21 6.75v6.75A2.25 2.25 0 0 1 18.75 16.5H5.25A2.25 2.25 0 0 1 3 14.25V6.75z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 9.75h18"/></svg>' },
                    { route: '<?php echo e(route('admin.reports.index')); ?>', label: 'Laporan', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 9V5m0 0L8 9m4-4v14m-4-7h8"/></svg>' },
                ],
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('a[href^="<?php echo e(url('admin')); ?>"]').forEach(link => {
...
</html><?php /**PATH D:\xampp\htdocs\UKK MODE SERIUS\skillhub\resources\views/layouts/admin.blade.php ENDPATH**/ ?>