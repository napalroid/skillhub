            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div>
                    ...
                    <p class="text-sm text-slate-500 leading-relaxed">
                        Marketplace jasa untuk lingkungan sekolah. Tawarkan keahlianmu atau temukan bantuan dari teman sekolah — dengan pembayaran escrow yang aman.
                    </p>
                </div>

                <div>
                    <h3 class="text-sm font-semibold text-slate-900 mb-4">Navigasi</h3>
                    <ul class="space-y-2">
                        <li><a href="{{ route('services.index') }}" class="text-sm text-slate-500 hover:text-blue-600 transition-colors">Jelajahi Jasa</a></li>
                        <li><a href="#cara-kerja" class="text-sm text-slate-500 hover:text-blue-600 transition-colors">Cara Kerja</a></li>
                        <li><a href="#escrow" class="text-sm text-slate-500 hover:text-blue-600 transition-colors">Sistem Escrow</a></li>
                        @guest
                            <li><a href="{{ route('register') }}" class="text-sm text-slate-500 hover:text-blue-600 transition-colors">Daftar Akun</a></li>
                        @endguest
                    </ul>
                </div>

                <div>
                    <h3 class="text-sm font-semibold text-slate-900 mb-4">Akun</h3>
                    <ul class="space-y-2">
                        @auth
                            <li><a href="{{ route('dashboard') }}" class="text-sm text-slate-500 hover:text-blue-600 transition-colors">Dashboard</a></li>
                            <li><a href="{{ route('orders.index') }}" class="text-sm text-slate-500 hover:text-blue-600 transition-colors">Pesanan Saya</a></li>
                            <li><a href="{{ route('services.create') }}" class="text-sm text-slate-500 hover:text-blue-600 transition-colors">Ajukan Jasa</a></li>
                        @else
                            <li><a href="{{ route('login') }}" class="text-sm text-slate-500 hover:text-blue-600 transition-colors">Masuk</a></li>
                            <li><a href="{{ route('register') }}" class="text-sm text-slate-500 hover:text-blue-600 transition-colors">Daftar</a></li>
                        @endauth
                    </ul>
                </div>
            </div>

            <div class="mt-10 pt-6 border-t border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-2">
                <p class="text-sm text-slate-400">&copy; {{ date('Y') }} SkillHub — Proyek UKK PPLG</p>
                <p class="text-xs text-slate-400">Pembayaran escrow · Negosiasi harga · Review pasca-transaksi</p>
            </div>
        </div>
    </footer>
