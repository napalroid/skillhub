...
<div class="min-h-screen bg-white">
    <div class="mx-auto max-w-[1400px] px-6 lg:px-8 py-12">
            ...
            <div class="grid grid-cols-3 gap-1">
                <div class="relative">
                    ...
                    <div class="pl-4">
                        ...
                        <div class="text-[52px] leading-none font-light tracking-[-0.03em] text-black">{{ $services->total() }}</div>
                    </div>
                </div>
                <div class="relative">
                    ...
                    <div class="pl-4">
                        ...
                        <div class="text-[52px] leading-none font-light tracking-[-0.03em] text-emerald-600">{{ $services->where('status', 'approved')->count() }}</div>
                    </div>
                </div>
                <div class="relative">
        ...
        <!-- Filter bar -->
        <form method="GET" action="{{ route('services.my') }}" class="mb-12">
            <div class="flex items-end gap-4">
                ...
                <div class="flex-1">
                    ...
                    <select name="subcategory" id="filterSubcategory" 
                        ...
                        @foreach($subcategories as $sub)
                            <option value="{{ $sub->id }}" data-category="{{ $sub->category_id }}" {{ request('subcategory') == $sub->id ? 'selected' : '' }}>{{ $sub->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <a href="{{ route('services.my') }}" class="px-6 py-2.5 border border-black/15 text-sm font-medium hover:border-black transition-colors">
                    Reset
                </a>
            </div>
        </form>
            ...
            <article class="group border-t border-black/10 pt-4">
                <div class="flex gap-4 items-center">
                    ...
                    <div class="text-sm text-black/20 w-6 shrink-0">
                        {{ str_pad($services->firstItem() + $index, 2, '0', STR_PAD_LEFT) }}
                    </div>
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 text-[10px] tracking-[0.06em] uppercase text-black/40 mb-1">
                            <span>{{ $service->subcategory->category->name ?? 'N/A' }}</span>
                    ...
                    <div class="flex items-center gap-3 text-xs text-black/50 shrink-0">
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            <span>{{ number_format($service->reviews_avg_rating ?? 0, 1) }}</span>
                        </div>
                        @endif
                    </div>
