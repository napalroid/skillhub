            ...
            <section>
                <div class="rounded-[1.75rem] border border-gray-200/80 bg-gray-50 p-2">
                    <div class="rounded-[1.4rem] bg-white p-6 sm:p-8">
                        <form method="POST" action="{{ route('orders.store') }}" x-data="{ sending: false }" @submit="sending = true" class="space-y-6" id="orderForm">
                            @csrf
                            ...
                            <div class="border-t border-gray-100 pt-6">
                                <h3 class="mb-4 text-sm font-bold text-gray-700">Informasi Booking</h3>
                                <div class="space-y-4">
                                    @foreach($service->booking_config['fields'] as $field)
                                    <div>
                                        <label for="booking_{{ $field['name'] }}" class="mb-2 block text-sm font-bold text-gray-700">
                                            {{ $field['label'] }}
                                        ...
                                        @if($field['type'] === 'textarea')
                                            <textarea 
                                                ...
                                                class="w-full resize-y rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm leading-6 text-gray-900 outline-none transition placeholder:text-gray-400 focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100">{{ old('booking_data.' . $field['name']) }}</textarea>
                                        ...
                                        @elseif($field['type'] === 'select')
                                            <select 
                                                ...
                                                id="booking_{{ $field['name'] }}"
                                                {{ $field['required'] ? 'required' : '' }}
                                                class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100">
                                        ...
                                        @elseif($field['type'] === 'number')
                                            <input 
                                                ...
                                                max="{{ $field['max'] ?? '' }}"
                                                {{ $field['required'] ? 'required' : '' }}
                                                class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-900 outline-none transition placeholder:text-gray-400 focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100">
                                    ...
                                    @endforeach
                                     ...
                                     @if($service->booking_config['time_slots_enabled'] ?? false)
                                     <div class="border-t border-gray-100 pt-6">
                                         <h3 class="mb-4 text-sm font-bold text-gray-700">Pilih Jam Booking</h3>
                                         
                                         @php
                                             $allSlots = \App\Models\ServiceTimeSlot::where('service_id', $service->id)
                                         ...
                                         <div class="grid gap-2 sm:grid-cols-2">
                                             ...
                                             <label class="flex cursor-pointer items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2 hover:border-blue-400 hover:bg-blue-50">
                                                 <input type="radio" name="time_slot_id" value="{{ $slot->id }}" class="h-4 w-4 text-blue-600 focus:ring-blue-500">
                                                 <div>
                                                     <p class="text-sm font-bold text-gray-900">{{ \Carbon\Carbon::parse($slot->date)->format('d M') }} - {{ $slot->time_start }} - {{ $slot->time_end }}</p>
                                                 </div>
                                             </label>
                                             @endforeach
                                         </div>
                                         ...
                                         <div class="rounded-lg border border-dashed border-gray-300 bg-gray-50 p-6 text-center">
                                             <p class="text-sm text-gray-500">Tidak ada slot waktu tersedia untuk saat ini.</p>
                                         </div>
                                         @endif
                                     </div>
                                     @endif
