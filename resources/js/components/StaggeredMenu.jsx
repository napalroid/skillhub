...
function AccountControl({ authenticated, userName, avatarUrl, profileUrl, servicesMyUrl, loginUrl, registerUrl, logoutUrl, csrfToken, notifications: initialNotifications, notificationsUrl, readAllUrl, menuOpen, onOpenMenu }) {
    ...
    return <div className={`stagger-account${dark}`} ref={accountRef}>
        <button type="button" className="stagger-account-trigger" onClick={() => { setProfileOpen((current) => !current); onOpenMenu(false); }} aria-expanded={profileOpen} aria-haspopup="menu" title={authenticated ? `Akun ${userName}` : 'Buka pilihan akun'}>
            {authenticated ? <span className="stagger-avatar">{initial}</span> : <span className="stagger-user-icon" aria-hidden="true" />}
        ...
        <AnimatePresence>{profileOpen && <motion.div className="stagger-account-popover" role="menu" initial={{ opacity: 0, y: -8, scale: .97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: .97 }} transition={{ duration: .18, ease: [0.16, 1, 0.3, 1] }}>
            {authenticated ? <>
                ...
                <div className="stagger-account-all">
                ...
                <div className="stagger-account-actions"><a href={servicesMyUrl} role="menuitem">Jasa Saya</a><a href={profileUrl} role="menuitem">Edit profil</a><form method="POST" action={logoutUrl}><input type="hidden" name="_token" value={csrfToken} /><button type="submit" role="menuitem">Log out</button></form></div>
...
if (menuRoot) {
    console.log('✅ StaggeredMenu: Found menu root element');
    console.log('✅ React:', !!React);
    console.log('✅ Framer Motion:', !!motion);
    
    const isAdmin = menuRoot.dataset.isAdmin === 'true';
    const adminDashboardUrl = menuRoot.dataset.adminDashboard;
    
    const menuItems = [
        { label: 'Home', href: menuRoot.dataset.home },
    ...
    if (isAdmin && adminDashboardUrl) {
        menuItems.push({ label: 'Admin', href: adminDashboardUrl });
    }
    ...
    console.log('🎨 Mounting StaggeredMenu component...');
    createRoot(menuRoot).render(<StaggeredMenu links={links} account={account} />);
    console.log('✅ StaggeredMenu mounted successfully');
} else {
    console.warn('⚠️ StaggeredMenu: Element #skillhub-staggered-menu not found');
}
