...
const ActiveServiceContext = createContext({ activeIndex: null, setActiveIndex: () => {} });
...
function ServiceProduct({ service, index }) {
    ...
    const anotherProductIsHovered = activeIndex !== null && !isHovered;
    ...
    return <motion.article
        ...
        animate={reduceMotion ? undefined : { scale: anotherProductIsHovered ? .965 : isHovered ? 1.025 : 1, y: isHovered ? -5 : 0 }}
    ...
    >
        <a href={service.url} className="service-product-link">
            ...
            {hasPortfolio && <div className="service-product-portfolio" aria-label="Portofolio jasa">
                {service.portfolios.slice(0, 3).map((image, portfolioIndex) => <img key={image} src={image} alt="" loading="lazy" decoding="async" className={isHovered && portfolioIndex === 0 ? 'is-active' : ''} />)}
            </div>}
...
function FeaturedServices({ services, marketplaceUrl }) {
    ...
    return <ActiveServiceContext.Provider value={{ activeIndex, setActiveIndex }}><section className="featured-marketplace" aria-label="Jasa unggulan SkillHub" onMouseLeave={() => setActiveIndex(null)}>
        <style>{`
        ...
        `}</style>
        <style>{`
            .featured-marketplace-wrap{max-width:1320px}.featured-marketplace-grid{gap:6px;background:transparent}.service-product{z-index:0;border:1px solid rgba(8,8,8,.16);transform-origin:center top}.service-product.is-hovered{z-index:2;border-color:#080808;outline:1px solid #080808;outline-offset:-1px;box-shadow:0 12px 28px rgba(8,8,8,.16)}.service-product-media{aspect-ratio:16/9;background:#e7eaeb}.service-product-image{object-fit:cover}.service-product-portfolio{display:none}.service-product.is-hovered .service-product-portfolio{display:flex}.service-product-copy{min-height:132px}.service-product-portfolio-label{display:none}.service-product.is-hovered .service-product-portfolio-label{display:block}@media(max-width:760px){.featured-marketplace-grid{gap:4px}.service-product-copy{min-height:128px}}@media(prefers-reduced-motion:reduce){.service-product{transform:none!important}}
        `}</style>
    ...
    </section></ActiveServiceContext.Provider>;
}
