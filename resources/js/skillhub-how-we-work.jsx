...
function HowWeWork() {
    ...
    return <section id="how-we-work" className="how-we-work-section">
        ...
        <div className="how-we-work-wrap">
            ...
            <div className="how-we-work-gallery" role="tablist" aria-label="Tahapan transaksi SkillHub">{steps.map((step, index) => { const Icon = step.icon; const isActive = activeIndex === index; return <button key={step.title} type="button" role="tab" aria-selected={isActive} className={`how-we-work-panel ${isActive ? 'is-active' : ''}`} onClick={() => setActiveIndex(index)} onMouseEnter={() => setActiveIndex(index)}><span className="how-we-work-panel-inner"><span className="how-we-work-step">LANGKAH 0{index + 1}</span><span className="how-we-work-icon"><Icon aria-hidden="true" /></span><strong className="how-we-work-title">{step.title}</strong>{isActive && <span className="how-we-work-description">{step.text}</span>}</span></button>; })}</div>
